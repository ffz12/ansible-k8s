...
`ansible-pro` 是一个用于 Linux 环境初始化和批量部署的 Ansible 工具集，支持 Docker、GPU、Harbor、Kubernetes、etcd、Nginx-HA、HAProxy、Ceph 等常见组件的部署与管理。
...
- [前置条件](#前置条件)
- [快速开始](#快速开始)
- [常用 Playbook](#常用-playbook)
- [GPU 相关操作](#gpu-相关操作)
- [Harbor 与 cri-dockerd](#harbor-与-cri-dockerd)
- [Kubernetes / etcd 部署流程](#kubernetes--etcd-部署流程)
- [监控与插件](#监控与插件)
- [MPI Job](#mpi-job)
...
- [故障排除](#故障排除)
- [清理与卸载](#清理与卸载)
- [Ubuntu 网络示例](#ubuntu-网络示例)
- [Git 提交常用命令](#git-提交常用命令)
- [贡献](#贡献)
- [作者](#作者)
- [更新日志](#更新日志)
- [致谢](#致谢)
...
```bash
git clone https://github.com/your-username/ansible-pro.git
cd ansible-pro
```
...
- 控制节点已安装 Ansible
- SSH 可连通目标主机，并已配置免密登录
- 目标节点已安装 Python 3
- 控制节点与目标节点网络连通
- `inventory/hosts` 已正确配置主机组和变量
...
[harbor]
harbor-node ansible_host=192.168.1.12

[gpu]
gpu-node ansible_host=192.168.1.13
```
...
```bash
#设置/tmp为777
ansible cluster -m shell -a "chmod 777 /tmp"
#环境初始化
ansible-playbook -i inventory/hosts playbook/init.yaml
```
### 删除原有docker
```bash
ansible cluster -m shell -a "apt remove docker.io -y "
ansible cluster -m shell -a "apt remove nginx -y "
ansible cluster -m shell -a "apt remove containerd -y "
ansible cluster -m shell -a "systemctl unmask docker.socket docker.service containerd.service"
...
```bash
ansible-playbook -i inventory/hosts playbook/harbor-install.yaml
```
...
```bash
ansible-playbook -i inventory/hosts playbook/k8s-init-master.yaml
```

### 添加 Kubernetes Master

```bash
ansible-playbook -i inventory/hosts playbook/k8s-add-master.yaml
```

### 添加 Kubernetes 节点

```bash
ansible-playbook -i inventory/hosts playbook/k8s-add-node.yaml
```

### 设置主机名

```bash
ansible-playbook -i inventory/hosts playbook/host-set.yaml
```
...
```bash
sh NVIDIA-Linux-x86_64-580.126.09.run
...
```bash
ansible k8s_node -m shell -a "lsmod | grep nvidia_peermem"
echo "nvidia-peermem" | sudo tee /etc/modules-load.d/nvidia-peermem.conf
modprobe -v nvidia-peermem
```

### 检查 nvidia-fabricmanager
...
在 `inventory/group_vars/all.yaml` 中配置 Harbor 实例：

```yaml
harbor_instances:
  - { dns: "harbor.unisound.ai", ip: "172.31.72.107" }
```

### CRI-Docker 配置

修改 `inventory/group_vars/all.yaml` 中 `pause` 变量：
...
```bash
ansible-playbook -i inventory/hosts playbook/cri-dockerd.yaml
ansible docker -m shell -a "systemctl status cri-docker"
```

## Kubernetes / etcd 部署流程

### 示例 hosts

```ini
[k8s_master]
jzcpu101 ansible_host=172.31.72.101
jzcpu102 ansible_host=172.31.72.102
jzcpu103 ansible_host=172.31.72.103
...
#检查nvidia-fabricmanager.service
ansible gpu -m shell -a "systemctl status nvidia-fabricmanager.service"
ansible gpu -m shell -a "systemctl is-enabled  nvidia-fabricmanager.service"
#检查nvidia_peermem
ansible gpu -m shell -a "lsmod | grep nvidia_peermem"
#设置开机加载  role为gpu
ansible gpu -m shell -a 'echo "nvidia-peermem" | sudo tee /etc/modules-load.d/nvidia-peermem.conf'
ansible gpu -m shell -a 'modprobe -v nvidia-peermem'
#安装cri-docker role为docker
ansible-playbook -i inventory/hosts playbook/cri-dockerd.yaml
#添加k8s node节点
ansible-playbook playbook/k8s-add-node.yaml 
#更新主机名 
#jzgpu17 ansible_host=172.31.72.17 这台为调试机，更新主机名时要把其打开
...
```bash
# GPU 节点打标签
kubectl label node gpu24 kubernetes.io/role=H100
kubectl label node jzgpu17 gpu=true
k label no  `k get node |grep none |awk '{print $1}'` kubernetes.io/role=H100
```
...
- 验证 SSH 密钥：`ssh-keygen -l -f ~/.ssh/id_rsa.pub`
- 测试连接：`ssh -T user@host`

#### Ansible 执行失败
- 检查 Ansible 版本：`ansible --version`
- 验证 inventory 文件：`ansible -i inventory/hosts all --list-hosts`
- 启用详细输出：添加 `-vvv` 参数

#### GPU 相关问题
- 检查 NVIDIA 驱动：`nvidia-smi`
- 验证容器工具包：`docker run --gpus all nvidia/cuda:11.0-base nvidia-smi`

#### Kubernetes 部署问题
- 检查节点状态：`kubectl get nodes`
- 查看 Pod 日志：`kubectl logs <pod-name> -n <namespace>`
- 验证集群配置：`kubectl cluster-info`
#### 镜像同步方法
```bash
...
NEW_USER_PASS="admin:Harbor12345"
...
### v1.0.0 (2024-04-16)
- 初始版本发布
- 支持 Docker、GPU、Harbor、Kubernetes、etcd 等组件部署
- 添加 MPI Job 支持
- 集成 Volcano 调度器
- 支持 k8s 原生调度器 binpack 策略
...
- [Volcano](https://volcano.sh/) - Kubernetes 批处理调度器
- [NVIDIA](https://www.nvidia.com/) - GPU 技术和工具
- [MPI Operator](https://github.com/kubeflow/mpi-operator) - Kubernetes MPI 作业操作器
